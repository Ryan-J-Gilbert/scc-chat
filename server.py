"""
FastAPI server for the HPC RAG Chatbot - Simplified stateless version
"""
# http://0.0.0.0:8000
import os
import json
...
async def chat_endpoint(request: ChatRequest): # , token: SessionToken):
    try:
        # verify token
        jwt = decode_token(request.token)

        print("TODO: HANDLE TOKEN VERIFY")

        logger.log_user_message(jwt["chat_id"], request.messages[-1].get("content", ""), jwt["username"], len(encoding.encode(request.messages[-1].get("content", ""))))

        # Manage context window size
        messages_dict = manage_context_window(request.messages)
        
        # Check if system message exists - add logging in production
        system_message_exists = any(msg.get("role") == "system" for msg in messages_dict)
        if not system_message_exists:
            raise HTTPException(status_code=500, detail=f"WARNING: No system message found in request")
        ...
        if assistant_message.tool_calls:
            ...
            logger.log_tool_call(jwt["chat_id"], {
