...
class ChatbotClient:
    ...
    def start_session_server(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.server_url}/start_session", json={"username": username}
            )
            if response.status_code == 200:
    ...
    def send_message(self, user_message: str, stream: bool = False) -> str:
        ...
        # Prepare request
        request_data = {"messages": self.messages, "stream": stream, "token": self.JWT}
        ...
        try:
            ...
            if stream:
                ...
                response = requests.post(
                    f"{self.server_url}/chat", headers=self.headers, json=request_data
                )
    ...
    def run_chat_loop(self):
        ...
        initial_string = (
            "Hello! I can help you with questions about the university's Shared Computing Cluster. "
            'You can ask specific questions like, "How to run my Python script with 16 cores?", '
            "but please do not include any sensitive data in your message! What would you like to know?"
        )
        print(
            format_message(
                initial_string,
        ...
        while True:
            try:
                ...
                end_time = time.time()
                self.debug_print(
                    f"Response received in {end_time - start_time:.2f} seconds"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--nostream", action="store_true", help="Disable streaming responses"
    )
...
if __name__ == "__main__":
    ...
    client = ChatbotClient(
        ...
        enable_logging=False,  # not args.no_log
    )
    ...
    if client.check_server_health():
        ...
        print(
            f"Could not connect to server at {args.server}. Please check if the server is running."
        )
        sys.exit(1)
