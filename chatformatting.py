...
def format_text_with_markdown(
    text, terminal_width=80, indent="", hyperlinks_supported=True
):
    ...
    for segment_type, content in link_segments:
        if segment_type == "text":
            ...
            # Bold: **text**
            processed_item = re.sub(
                r"\*\*(.*?)\*\*", f"{BOLD}\\1{RESET}", processed_item
            )
            # Italic: *text* or _text_
            processed_item = re.sub(
                r"(\*|_)(.*?)\1", f"{ITALIC}\\2{RESET}", processed_item
            )
            # Strikethrough: ~~text~~
            processed_item = re.sub(
                r"~~(.*?)~~", f"{STRIKETHROUGH}\\1{RESET}", processed_item
            )
            # Code: `text`
            processed_item = re.sub(
                r"`(.*?)`", f"{BG_BLACK}{WHITE}\\1{RESET}", processed_item
            )
    ...
    while i < len(lines):
        ...
        while j < len(lines):
            ...
            if (
                ...
                and not re.match(r"^(\*{3,}|-{3,}|_{3,})\s*$", next_line)
...
def format_list_item(
    text, indent_level=0, ordered=False, index=None, base_indent="", terminal_width=80
):
...
def format_message(
    ...
    for element_type, element_data in elements:
        if element_type == "text":
            ...
            formatted_element = format_blockquote(
                element_data, base_indent, terminal_width
            )
            formatted_content.append(formatted_element)
...
def old_print_streaming_response(
    response_stream, terminal_width=80, assistant_color=GREEN
):
        ...
        if hasattr(chunk, "choices") and chunk.choices and len(chunk.choices) > 0:
            ...
            if content:
                ...
                if "\n" in buffer or ". " in buffer or "? " in buffer or "! " in buffer:
                    ...
                    # Bold
                    formatted_buffer = re.sub(
                        r"\*\*(.*?)\*\*", f"{BOLD}\\1{RESET}", formatted_buffer
                    )
                    # Italic
                    formatted_buffer = re.sub(
                        r"(\*|_)(.*?)\1", f"{ITALIC}\\2{RESET}", formatted_buffer
                    )
                    # Code
                    formatted_buffer = re.sub(
                        r"`(.*?)`", f"{BG_BLACK}{WHITE}\\1{RESET}", formatted_buffer
                    )
