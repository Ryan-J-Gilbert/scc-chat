...
class ChatMessage(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        if self.sender == "user":
            ...
            yield Vertical(header, content, classes="bot-message")
            yield Static("", classes="spacer")
...
class ChatDisplay(Vertical):
    ...
    def add_message(self, sender: str, message: str) -> None:
        ...
        msg = ChatMessage(sender, message, timestamp)
        self.mount(msg)
        # Scroll to bottom
...
class SupportChatbot(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("Ready to help!", id="status")
        yield Footer()
    ...
    def on_mount(self) -> None:
        ...
        # Set the theme to inherit our custom colors
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "send-button":
            # Trigger the same handler as Ctrl+J
            self.post_message(ChatInput.Submitted(self.message_input))
    
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        """Handle when user presses Ctrl+Enter in the input."""
        ...
        # Update status
        self.status.update("Bot is typing...")
        
        # Simulate bot processing (replace with actual chatbot logic)
        await asyncio.sleep(1)
        
        # Generate bot response (replace with your actual chatbot)
        bot_response = self.generate_response(message)
        
        # Add bot response
        self.chat_display.add_message("bot", bot_response)
        
        # Update status
        self.status.update("Ready to help!")
    
    async def action_send_message(self) -> None:
        """Handle sending a message via binding (fallback)."""
        # Trigger the same handler as Ctrl+Enter
        self.post_message(ChatInput.Submitted(self.message_input))
    
    async def on_input_submitted(self, event) -> None:
        """Handle when user submits a message (legacy support)."""
        # This is now handled by action_send_message
        pass
    ...
    def generate_response(self, user_message: str) -> str:
        ...
        # Simple keyword-based responses (replace with your actual chatbot)
...
Just type your question and I'll do my best to assist you!"""
    ...
    def action_clear(self) -> None:
        ...
        self.status.update("Chat cleared - Ready to help!")
