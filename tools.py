...
def hybrid_retrieve(collection, query, n_results_qa=2, n_results_article=2):
    ...
    if qa_results['documents'] and len(qa_results['documents'][0]) > 0:
        for i, (doc, metadata) in enumerate(zip(qa_results['documents'][0], qa_results['metadatas'][0])):
            retrieved_docs["qa_documents"].append({
                "content": doc,
                "source": metadata.get("source", "Unknown"),
                "distance": qa_results['distances'][0][i] if 'distances' in qa_results else None
            })
    
    # Process article results
    if article_results['documents'] and len(article_results['documents'][0]) > 0:
        for i, (doc, metadata) in enumerate(zip(article_results['documents'][0], article_results['metadatas'][0])):
            retrieved_docs["article_documents"].append({
                "content": doc,
                "source": metadata.get("source", "Unknown"),
                "title": metadata.get("title", "Unknown"),
                "distance": article_results['distances'][0][i] if 'distances' in article_results else None
            })
